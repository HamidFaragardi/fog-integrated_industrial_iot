...
import fogcloudproject.*;



public class Experiment {
List<PeriodicTaskSet>tasksetList=new ArrayList<PeriodicTaskSet>();
List<VM>vmList=new ArrayList<VM>();
	...
	*/
	...
	/**
	 * This function creates random VMs (also referred as components)
	 * Each VM consists of a random taskset
	 * @param newExp if false it binds the outout to the existing output files
	 * @param total_vms total number of VMs
	 */
	
        public List<PeriodicTaskSet> RandomComponents(boolean newExp, int total_vms,Bandwidth bandwidth)
	{
		
		double maxTaskUtil = 0.95;
		int minTaskPeriod = 10;
		int deltaTaskPeriod = 300;
		//int M = 5;
		//Log log = new Log("random_vms");
		//if(newExp)
		//	log.DeleteFiles();
		...
		{
			System.out.printf("%d, ", i);
			PeriodicTaskSet myTasks = new PeriodicTaskSet();
                        myTasks.setIndex(i);
			myTasks.setPeriodRange(minTaskPeriod, deltaTaskPeriod);
                        int t_class=(new Random()).nextInt(3);
                        if(t_class==0)
                        {
                            myTasks.CreateRandomTaskSet(20,bandwidth);
                        }
                        else if(t_class==1)
                        {
                            myTasks.CreateRandomTaskSet(30,bandwidth);
                        }
                        else
                        {
                            myTasks.CreateRandomTaskSet(40,bandwidth);
                        }
			...
			//comp.setPi(Pi);
			//double Theta = comp.getMPR().getTheta();
			//comp.FFDecompose();
			//comp.getTaskSet().setIndex(i);
			//Logging
                        //VM newVM=new VM(i, Pi*2, Theta/Pi, (new MPR()).calcMinimalInterfaceEdited(myTasks,Pi), 0);//(int)Math.random()*100+15);
			//vmList.add(newVM);
                        //String util = Double.toString(myTasks.getUtilization());
			//String MPROverhead = Double.toString(comp.getMPROverhead());
			String cntTasks = Integer.toString(myTasks.getSize());
			//String currentPi = Integer.toString(Pi);
			//String currentTheta = Double.toString(Theta);
            myTasks.calculateRam();
            tasksetList.add(myTasks);
			//log.WriteLog(myTasks, util, cntTasks, currentPi, currentTheta);
                      //log.WriteLog(myTasks, util, cntTasks, currentPi, currentTheta,Integer.toString((new MPR()).calcMinimalInterfaceEdited(myTasks,Pi)));
		}

		System.out.println("random VM experiment ended");
                return tasksetList;
        ...
        */
        ...
        public VM VMSpecify(PeriodicTaskSet myTask,int device_index,int MValue)
	{
            if(myTask.getTaskList().size()==0)
            {
                VM newVM=new VM(device_index, 0, 0,0,0, 0,device_index,true);
                return newVM;
            }
		...
		//int deltaTaskPeriod = 100;
		int M = MValue;
		Log log = new Log("random_vms",device_index);
		//System.out.println("\n random VM started");
		//List<TaskSet>tasksetList=new ArrayList<TaskSet>();
                ...
                Component comp = new Component(myTask, M,device_index);
                int Pi = myTask.getMinPeriods()/2;
                comp.setPi(Pi);
                double Theta = comp.getMPR().getTheta();
                comp.getTaskSet().setIndex(0);
                //Logging
                int max_core=(new MPR()).calcMinimalInterfaceEdited(myTask,Pi,device_index);
                if(max_core>18)
                    max_core=18;
                double utilization=Theta/Pi;
                if(utilization==0)
                    utilization=myTask.getUtilization_on_specefic_Device(device_index);
                /*if(utilization>max_core)
                {
                    utilization=max_core;
                }*/
                VM newVM=new VM(0, Pi*2, utilization, max_core,myTask.getDataMass(), 0,device_index,myTask.Is_Schedulable(device_index));//(int)Math.random()*100+15);
                //vmList.add(newVM);
                newVM.ram = myTask.getTotal_ram();
                String util = Double.toString(myTask.getUtilization_on_specefic_Device(device_index));
                //String MPROverhead = Double.toString(comp.getMPROverhead());
                String cntTasks = Integer.toString(myTask.getSize());
                String currentPi = Integer.toString(Pi);
                String currentTheta = Double.toString(Theta);
                return newVM;

...
public class Log {
	...
	public Log(String inFilename,int device_index) {
		logPath = Paths.get(inFilename+".csv");
		//logPath = Paths.get("C:\\Users\\nmd01\\Dropbox\\My Papers\\Paper - 15.1 - RTS 2014\\Code\\JavaProgram\\log\\"+inFilename+".csv");
		taskSetLogPath = Paths.get(inFilename+"Tasks.csv");
		statusLogPath = Paths.get("status.txt");
	...
	private String convertTaskSet2String(PeriodicTaskSet taskSet)
	{
		String out = "";
		...
		for(int i=0;i<taskSet.getSize();i++)
		{
			...
			out += "\n" + task.index + sep;
			out += task.getExecution_time_on_specific_device(device_index) + sep;
			out += task.period + sep;
		}
		return out + "\n";
	}
	private void logTaskSet(PeriodicTaskSet taskSet, Path path)
	{
	...
	public void WriteLog(PeriodicTaskSet taskSet, String... args)
	{
		logTaskSet(taskSet, taskSetLogPath);
		logStatus(taskSet.getIndex(), taskSet.getUtilization_on_specefic_Device(device_index));
	...
	public RTSystem ReadSystem(long id)
	{
		...
		while(i<file.size())
		{
			...
			i++;
			PeriodicTaskSet taskSet = new PeriodicTaskSet();
			while(i<file.size() && (int)(double)file.get(i).length > 1) //line is a task
			{
				...
				task.execution_time[device_index] = (int)(double)file.get(i)[1];
				task.period = (int)(double)file.get(i)[2];
				task.absolute_deadline = task.period;

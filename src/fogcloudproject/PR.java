...
public class PR {
	 ...
	 * @return
	 */
	private double dbf(PeriodicTaskSet taskSet, long t)
	{
		double demand = 0;
	    for(int i=0;i<taskSet.getSize();i++)
	    {
	    	PeriodicTask taski = taskSet.getTask(i);
	    	demand += Math.floor(t/taski.period)*taski.getExecution_time_on_specific_device(i);
	    }
	 ...
	 * @param taskSet
	 */
	public void calcInterface(PeriodicTaskSet taskSet)
	{
		int tmpTheta = 0;
		double demand = 0;
		Theta = 0;
	...
	private long getNetTime(PeriodicTaskSet taskSet, long t)
	{
		PeriodicTask taski = taskSet.getTask(0);
